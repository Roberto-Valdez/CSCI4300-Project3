...
def index():
	...
	tableResults = cur.execute("MYSQL QUERY") #insert query here and use where id = employeeID from up above                                                     
	if tableResults > 0:  #check to see if there are results associated with the employeeID number                                                               
	    ...
	    return json.dumps(json_data) #returns json data, still need to figure out how to link to HTML and possibly parse using jQuery.getJSON() functions        
